...
# This line uses curl to silently download the content of the version.txt file from the specified GitHub repository. The --max-time 30 option sets a 30-second timeout, and the -4 option forces the use of IPv4.
Temp_Value=$(curl --silent --max-time 30 -4 https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel/main/version.txt)
# These lines extract specific substrings from the downloaded content:
# Panel_Version extracts 3 characters starting from the 12th position.
Panel_Version=${Temp_Value:12:3}
# Panel_Build extracts 1 character starting from the 25th position.
Panel_Build=${Temp_Value:25:1}
# This line constructs the branch name by prefixing the version and build with "v" and separating them with a dot.
...
echo -e "\nChecking root privileges..."
  ...
  if [[ $(id -u) != 0 ]] >/dev/null; then
    ...
    echo -e "\e[31msudo su -c \"sh <(curl https://github.com/josephgodwinkimani/cyberpanel || wget -O - https://github.com/josephgodwinkimani/cyberpanel)\"\e[39m"
    exit 1
  else
