...
'''
    ...
    @staticmethod
    def find_open_port(start_port=8082, end_port=8090):
        for port in range(start_port, end_port + 1):
            if not Upgrade.is_port_open(port):
                return port
    ...
    def upgrade_crowdsec():
        try:
            ...
            if not Upgrade.is_port_open(8082):
                Upgrade.stdOut('Port 8082 is not open. Finding an available port...' + " [upgrade_crowdsec]")
                new_port = Upgrade.find_open_port()
                if new_port is None:
                    Upgrade.stdOut("No available ports found [upgrade_crowdsec]")
                else:
                    Upgrade.stdOut('Using available port: {}'.format(new_port) + " [upgrade_crowdsec]")
            else:
                new_port = 8082
            ...
            if not os.path.exists("/etc/crowdsec"):
                ...
                # Determine the operating system and set the upgrade command
                ...
                # List of collections to upgrade
                collections = [
                ...
                for collection in collections:
                    command = f"cscli collections upgrade {collection}"
                    Upgrade.executioner(command, 0)
    ...
    @staticmethod
    def upgrade_fail2ban():
        try:
            if not os.path.exists("/etc/fail2ban"):
                Upgrade.stdOut("Upgrade Fail2Ban")
                # Upgrade CentOS 7/CentOS 8
                if Upgrade.FindOperatingSytem() == CENTOS7 or Upgrade.FindOperatingSytem() == CENTOS8:
                    command = "yum -y upgrade fail2ban"
                    Upgrade.executioner(command, 0)
                else:  # Upgrade Ubuntu
                    command = "apt -y upgrade fail2ban"
                    Upgrade.executioner(command, 0)

                # Additional configuration or actions can be added here if necessary
                # For example, restarting the Fail2Ban service after upgrade
                command = "systemctl restart fail2ban"
                Upgrade.executioner(command, 0)

            else:  # Install
                # Determine the operating system and determine the package manager based on the distribution
                os_type = Upgrade.FindOperatingSytem()
                if os_type in (CENTOS7, CENTOS8):
                    command = 'yum -y install fail2ban'
                    Upgrade.executioner(command, 0)
                else:
                    command = 'DEBIAN_FRONTEND=noninteractive apt-get -y install fail2ban'
                    Upgrade.executioner(command, 0)

                # Configure Fail2Ban for intrusion prevention
                with open('/etc/fail2ban/jail.local', 'w') as config_file:
            ...
            Upgrade.stdOut(str(msg) + " [upgrade_fail2ban]")
    ...
    def download_and_install_rclone():
        try:
            ...
            if os.path.exists("/usr/bin/rclone"):
                ...
                if Upgrade.FindOperatingSytem() == CENTOS7 or Upgrade.FindOperatingSytem() == CENTOS8:
                    command = "wget https://downloads.rclone.org/v1.68.2/rclone-v1.68.2-linux-amd64.rpm"
                    Upgrade.executioner(command, 0)
                    command = "yum localinstall rclone-v1.68.2-linux-amd64.rpm -y"
                    Upgrade.executioner(command, 0)
                else:  # Upgrade ubuntu
                    command = "wget https://downloads.rclone.org/v1.68.2/rclone-v1.68.2-linux-amd64.deb"
                    Upgrade.executioner(command, 0)
