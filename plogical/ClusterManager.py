    ...
    def __init__(self, type):
        ...
        if os.path.exists(ClusterManager.ClusterFile):
            self.config = json.loads(
                open(ClusterManager.ClusterFile, 'r').read())
        elif os.path.exists(ClusterManager.CloudConfig):
            self.config = json.loads(
                open(ClusterManager.CloudConfig, 'r').read())

        self.type = type

    def PostStatus(self, message):
        try:
            finalData = {'name': self.config['name'], 'type': self.type,
                         'message': message, 'token': self.config['token']}
            # resp = requests.post(ClusterManager.LogURL,
        ...
        try:
            ...
            if self.type == 'Child':
                ...
                if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                    ClusterConfigFailover = ClusterConfigFailover.replace(
                        '/usr/lib/galera/libgalera_smm.so', '/usr/lib64/galera-4/libgalera_smm.so')
                ...
                writeToFile.write(
                    '*/5 * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/ClusterManager.py --function CreatePendingVirtualHosts --type Child\n')
                writeToFile.close()
            else:
                ...
                if ProcessUtilities.decideDistro() == ProcessUtilities.centos or ProcessUtilities.decideDistro() == ProcessUtilities.cent8:
                    ClusterConfigMaster = ClusterConfigMaster.replace(
                        '/usr/lib/galera/libgalera_smm.so', '/usr/lib64/galera-4/libgalera_smm.so')
                ...
                writeToFile = open(cronPath, 'a')
                writeToFile.write(
                    '*/%s * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/ClusterManager.py --function SyncNow --type Master\n' % (str(self.config['syncTime'])))
                writeToFile.write(
                    '*/3 * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/ClusterManager.py --function PingNow --type Master\n')
                writeToFile.close()
            ...
            try:
                ...
                newFireWallRule = FirewallRules(
                    name="mysqlpub", port='3306', proto="tcp")
                ...
                newFireWallRule = FirewallRules(
                    name="galery", port='4567', proto="tcp")
                newFireWallRule.save()
                ...
                newFireWallRule = FirewallRules(
                    name="galeryist", port='4568', proto="tcp")
                newFireWallRule.save()
                ...
                newFireWallRule = FirewallRules(
                    name="galerysst", port='4444', proto="tcp")
                newFireWallRule.save()
                ...
                newFireWallRule = FirewallRules(
                    name="galeryudp", port='4567', proto="udp")
                newFireWallRule.save()
                FirewallUtilities.addRule('udp', '4567', "0.0.0.0/0")

            except:
                self.PostStatus(
                    'Failed to add Firewall rules, manually open the required ports..')
...
password=%s""" % (rootdbpassword, rootdbpassword)
    ...
    def CreatePendingVirtualHosts(self):
        try:
            ...
            for website in Websites.objects.all():
                ...
                confPath = '%s/%s' % (ClusterManager.vhostConfPath,
                                      website.domain)
                if not os.path.exists(confPath):
                    self.PostStatus(
                        'Domain %s found in master server, creating on child server now..' % (website.domain))
                    virtualHostUtilities.createVirtualHost(website.domain, website.adminEmail, website.phpSelection, website.externalApp,
                                                           1, 1, 0, website.admin.userName, website.package.packageName, 0, '/home/cyberpanel/temp', 1, 0)
                    self.PostStatus(
                        'Domain %s successfully created.' % (website.domain))

            for childDomain in ChildDomains.objects.all():
                confPath = '%s/%s' % (ClusterManager.vhostConfPath,
                                      childDomain.domain)
                if not os.path.exists(confPath):
                    self.PostStatus('Child Domain %s found in master server, creating on child server now..' % (
                        childDomain.domain))
                    virtualHostUtilities.createDomain(childDomain.master.domain, childDomain.domain,
                                                      childDomain.phpSelection, childDomain.path, 1, 1, 0, childDomain.master.admin.userName, 0, 0)
                    self.PostStatus(
                        'Child Domain %s successfully created.' % (childDomain.domain))
            ...
            for website in os.listdir('/home'):
                ...
                if validators.domain(website):
                    if website not in allSite:
                        ...
                        command = 'rm -rf /%s/%s' % (
                            ClusterManager.vhostConfPath, website)
                        ProcessUtilities.normalExecutioner(command)
            ...
            self.PostStatus(
                'Failed to create pending vhosts, error %s [404].' % (str(msg)))

    def SyncNow(self):
        try:

            self.PostStatus(
                'Syncing data from home directory to fail over server..')

            command = "rsync -avzp -e 'ssh -o StrictHostKeyChecking=no -p %s -i /root/.ssh/cyberpanel' /home root@%s:/" % (
            ...
            self.PostStatus(
                'Failed to sync data, error %s [404].' % (str(msg)))
    ...
    def PingNow(self):
        try:
            ...
            self.PostStatus(
                'Failed to ping cloud for online status, error %s [404].' % (str(msg)))
    ...
    def DebugCluster(self):
        try:
            ...
            self.PostStatus(
                'Failed to debug cluster, error %s [404].' % (str(msg)))
    ...
    def UptimeMonitor(self):
        try:
            ...
            writeToFile.write(
                '*/3 * * * * /usr/local/CyberCP/bin/python /usr/local/CyberCP/plogical/ClusterManager.py --function Uptime --type All\n')
            writeToFile.close()
            ...
            logging.writeToFile(
                'Error while setting up Uptime cron. Error %s' % (str(msg)))

    def Uptime(self):
        try:
            finalData = {
                'name': self.config['name'], 'token': self.config['token']}
            resp = requests.post(ClusterManager.UptimeURL,
                                 data=json.dumps(finalData))
            if os.path.exists(ProcessUtilities.debugPath):
    ...
    def SyncToMaster(self):
        try:
            ...
            self.PostStatus(
                'Syncing data from home directory to Main server..')
