...
Watchdog="On"
Redis_Hosting="No"
Temp_Value=$(curl --silent --max-time 30 -4 https://raw.githubusercontent.com/josephgodwinkimani/cyberpanel/slim/version.txt)
...
echo -e "\n\e[31m-m\e[39m or \e[31m--minimal\e[39m : set to minimal mode which will not install PowerDNS, and Pure-FTPd"
echo -e "\n\e[31m-m pureftpd/powerdns\e[39m will do minimal install also with compoenent given"
echo -e "e.g.  \e[31m-m powerdns\e[39m will do minimal install also with PowerDNS"
...
if  [[ "$#" = "0" ]] || [[ "$#" = "1" && "$1" = "--debug" ]] || [[ "$#" = "1" && "$1" = "--mirror" ]]; then
  ...
  if [[ $1 = "help" ]]; then
    ...
    while [[ -n "${1}" ]]; do
      ...
      ;;
      -m | --minimal)
      if ! echo "$@" | grep -q -i "pureftpd\|powerdns" ; then       
        PowerDNS_Switch="Off"
        PureFTPd_Switch="Off"
...
echo -e "\nInstall Full service for CyberPanel? This will include PowerDNS and Pure-FTPd."
echo -e ""
